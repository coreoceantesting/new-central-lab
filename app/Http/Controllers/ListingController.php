...
class ListingController extends Controller
{
    ...
    public function resampling_list(Request $request)
    {
        $query = DB::table('patient_details')
        ->leftJoin('labs', 'patient_details.lab', '=', 'labs.id')
        ->leftJoin('main_categories', 'patient_details.main_category_id', '=', 'main_categories.id')
        ->where('patient_details.status', 'resampling')
        ->whereNull('patient_details.deleted_at');
        ...
        return view('admin.resamplingList',compact('patient_list','fromDate', 'toDate'));
    }

    public function update_status_recollection(Request $request, $id)
    {
        try
        {
            DB::beginTransaction();
            ...
            DB::table('patient_details')->where('patient_id', $id)->update([
                ...
                'recollect_by' => Auth::user()->name,
                'recollect_at'=> date('Y-m-d H:i:s')
            ]);
            DB::commit();

            return response()->json(['success'=> 'Patient Details Send For Recollection Successfully!']);
        }
        catch(\Exception $e)
        {
            return $this->respondWithAjax($e, 'updating', 'Patient status');
